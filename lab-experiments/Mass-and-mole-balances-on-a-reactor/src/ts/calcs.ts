...
const MW_Water = 18;
// const MW_Hydrogen = 2.02;
// const MW_Propene = 42.08;
const MW_Propanal = 58.08;
...
function calcAll(flow: number, temp: number): ProductStream {
    ...
    const m_liq = n_out / c_tot * (MW_Propanol * conc_propanol + MW_Water * conc_propene + MW_Propanal * conc_propanal);
    const n_vap = n_out / c_tot * (conc_propene + conc_propanal);

    // Finally, convert to volumetric flowrates
    const liq = m_liq / RHO;
    const vap = n_vap * R * 298.15 / P * 1000;
