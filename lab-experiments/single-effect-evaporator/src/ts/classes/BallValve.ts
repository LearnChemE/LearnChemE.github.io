export class BallValve {
    ...
    constructor(id: string, reverse=false, onTurn?: (state: boolean) => void) {
        ...
        this.coords = { x: reverse ? bbox.x + bbox.width * .925 : bbox.x + bbox.width * .075, y: bbox.y + bbox.height / 2 };
    ...
    private animate = () => {
        ...
        const frame = (time: number) => {
            ...
            // Request next frame
            if (newAngle !== target) requestAnimationFrame(frame);
        }
