  ...
  drawDimensionLine(pipeGroup, 
    ...
    drawDimensionLine(pipeGroup,  
      ...
      drawAngleArc(pipeGroup,
        ...
        const syringeX = startX - (pipeLength + 85) * Math.cos(angleRad);
        const syringeY = startY - (pipeLength + 85) * Math.sin(angleRad);
        
        syringeCenter = { x: syringe.bbox().cx, y: syringe.bbox().cy };
  ...
  `;
      ...
      export function drawDimensionLine(draw, start, end, arrowLen, textContent) {
        ...
        draw.polygon([ start.x, start.y,
          ...
          export function drawSyringe(draw, x, y, scale, flowRate = 1000, totalVolume = 1000) {
        ...
        switchGroup.click(() => {
          ...
          if (switchGroup.isOn) {
            ...
            syringeInterval = setInterval(() => {
              ...
              syringeProgress = Math.min(syringeProgress + deltaML, SYRINGE_TOTAL_VOLUME);
              updatePlungerPosition();

              if (syringeProgress >= SYRINGE_TOTAL_VOLUME) {
                clearInterval(syringeInterval);
              }
            }, SYRINGE_ANIM_INTERVAL);
          ...
          // (If you want to update pressure here, add code as needed)
        });
        // Remember pivot for reset
        switchGroup.pivot = { x, y, width, height };
        return switchGroup;
      }
...
export function reset() {
  ...
  if (syringe && syringe.initialWidth !== undefined) {
    syringe.liquid.attr({ width: syringe.initialWidth });
  }
  ...
  isRotated = false;
  // pressure.hide();
  pressure.text('0 Pa').center(975, 140);
}
