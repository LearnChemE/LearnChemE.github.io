...
import { AnimationSequence, createForceMemo, Fluids, resetSignal, transitionDescriptors, type MenuFluidType } from './globals';
...
import { ExtraLabels } from './components/ExtraLabels/ExtraLabels';
...
function App() {
  ...
  const [fluid, setFluid] = createSignal<MenuFluidType>(Fluids[0]);
  const { force, radius } = createForceMemo(rpm, fluid);
  ...
  <div class="canvas-container">
    ...
    <MagnifierProvider>
      <SVGCanvas width={160} height={190} defs={Defs}>
        ...
        <Arm transition={transitionTimer} force={force} scaleMax={() => fluid().scaleMax} />
        <Cylindrical transition={transitionTimer} fillColor={() => (fluid() as MenuFluidType).color} />
        <Fade transition={transitionTimer} />
        <ControlPanel transition={transitionTimer} disabled={() => !aniDone()} rpm={rpm} setRpm={setRpm} />
        <Streamlines rate={rpm} showing={showStream} shearRadius={radius} />

        <ExtraLabels transition={transitionTimer} />
