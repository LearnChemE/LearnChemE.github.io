...
import { conc_r, conc_w, particle_velocities, phi_r, phi_w, rhs_adv_diff } from './calcs';
...
describe('calcs.ts function visualization', () => {
    ...
    it("does a simple test run of the rhs function with the rk45 solver", async () => {
        ...
        for (let t=0;t<tmax;t+=dt) {
            ...
            y_cur = sol.y.at(-1)!;
            crs.push(y_cur.slice(0,nz).map(cr => phi_r(cr)));
            cws.push(y_cur.slice(nz).map(cw => phi_w(cw)));
            ts.push(t+dt);
        }
