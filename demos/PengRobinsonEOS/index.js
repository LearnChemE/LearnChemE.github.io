...
const Pc = [null, 3.797, 2.486]; // MPa (index 0 unused)
const Tc = [null, 425.2, 568.8]; // K (index 0 unused)
const omega = [null, 0.193, 0.396]; // (index 0 unused)
...
function calculateVLE(T, x1) {
    const x = [null, x1, 1 - x1]; // x[1] = x1, x[2] = 1 - x1
    const Pvle = x[1] * psat(1, T) + x[2] * psat(2, T);
    const y1 = x[1] * psat(1, T) / Pvle;
    const y = [null, y1, 1 - y1]; // y[1] = y1, y[2] = 1 - y1
...
function calculateAndUpdate() {
    ...
    x1Value.innerHTML = `<i>x</i><sub>1</sub> = ${x[1].toFixed(2)}`;
    y1Value.innerHTML = `<i>y</i><sub>1</sub> = ${y[1].toFixed(2)}`;
