...
function plotPoints(){
    ...
    yF = map(g.feedMoleFracs[0],0,1,g.ytip+g.dy,g.ytip);
    xtemp = map(g.feedMoleFracs[1],0,1,g.xtip+g.dx,g.xtip-g.dx); // x location along the bottom axis
    ...
    function feedCase(){
        let xtemp = map(g.feedMoleFracs[1],0,1,g.xtip+g.dx,g.xtip-g.dx);
        let x1, x2, y1;
    ...
    function solventCase(){
        ...
        noStroke(); textStyle(ITALIC); textSize(30); fill(0);
        text('S',xS-35,yS-15);
        pop();
    }
    ...
    function fractionLabel(){
        push();
        strokeWeight(1.5); textSize(25);
        rect(100,75,170,110);
        noStroke();
        if(g.mix == 'feed'){
            text('feed mole fractions',75,65);
            push();
            fill(0,0,255);
            text('solute = '+g.feedMoleFracs[0],112,105);
            fill(255,0,0);
            text('solvent = '+g.feedMoleFracs[1],108,135);
            fill(0,100,0);
            text('carrier = '+g.feedMoleFracs[2],112,165);
            pop();
        } else if (g.mix == 'solvent'){
            text('solvent mole fractions',65,65);
            push();
            fill(0,0,255);
            ...
            text('raffinate mole fractions',62,65);
            push();
            fill(0,0,255);
...
function mixingPoint(){
    ...
    let mixMoleFracs; // manually defining them since they're built in to mathematica
    if(g.feedMoleFracs[0] == .53){
        mixMoleFracs = [.21,.62,.17];
    } else if (g.feedMoleFracs[0] == .45){
        mixMoleFracs = [.18,.63,.19];
    } else if (g.feedMoleFracs[0] == .48){
        mixMoleFracs = [.19,.66,.15];
    }

    let xM, yM; // x and y locations of the mixing point
    yM = map(mixMoleFracs[0],0,1,g.ytip+g.dy,g.ytip);
    xtemp = map(mixMoleFracs[1],0,1,g.xtip+g.dx,g.xtip-g.dx);
    ...
    function fractionLabel(){
        ...
        rect(100,75,170,110);
        noStroke();
        text('mixing point mole fractions',45,65);
        push();
        fill(0,0,255);
        text('solute = '+mixMoleFracs[0],112,105);
        fill(255,0,0);
        text('solvent = '+mixMoleFracs[1],108,135);
        fill(0,100,0);
        text('carrier = '+mixMoleFracs[2],112,165);
        pop();
        pop();
    }
...
function determineE1(){
    ...
    function fractionLabel(){
        ...
        rect(100,75,170,110);
        noStroke();
        text('extract mole fractions',68,65);
        push();
        fill(0,0,255);
    ...
    fill(100); stroke(100);
    ellipse(xM,yM,2*g.radius);
    fill(255,100,51); stroke(255,100,51);
...
function operatingPoint(){
    ...
    push();
    strokeWeight(2); stroke(112,41,99);
    ...
    fill(255,100,51); stroke(255,100,51);
    ellipse(xe1,ye1,2*g.radius);
    fill(112,41,99); stroke(112,41,99);
