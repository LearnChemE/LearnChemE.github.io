...
export function calcAll() {
    ...
    function volumeConstPressureCase(x) {
        return (
            (totalMoles(x) * z.R * z.temperature) / z.pressureConstPressureCase
        );
    }
  ...
  */
    ...
    function extentReactionEQConstPressureCase(x) {
        return (
            z.kEQ -
            ((equilMolsB(x) * z.pressureConstPressureCase) / totalMoles(x)) **
                r /
                ((equilMolsA(x) * z.pressureConstPressureCase) / totalMoles(x))
        );
    }
    ...
    function extentReactionEQConstVolumeCase(x) {
        return (
            z.kEQ -
            ((equilMolsB(x) * pressureConstVolumeCase(x)) / totalMoles(x)) **
                r /
                ((equilMolsA(x) * pressureConstVolumeCase(x)) / totalMoles(x))
        );
    }
    ...
    z.extentConstPressureCase = findRoot(
        extentReactionEQConstPressureCase,
        0,
        4.999999
    );
    ...
    z.volumeConstPressureCase = volumeConstPressureCase(
        z.extentConstPressureCase
    );

    z.extentConstVolumeCase = findRoot(
        extentReactionEQConstVolumeCase,
        0,
        4.999999
    );
    ...
    z.pressureConstVolumeCase = pressureConstVolumeCase(
        z.extentConstVolumeCase
    );
}
