...
window.setup = function () {
    createCanvas(
        p5container.offsetWidth,
        p5container.offsetHeight,
        WEBGL
    ).parent(p5container);
    frameRate(60);
};
...
window.draw = function () {
    ...
    if (coeffB && coeffB.value != 1) {
        document.getElementById("equilibrium-text").textContent =
            `A ⇋ ${coeffB.value}B`;
  ...
  */
    ...
    translate(
        ...
        z.cylHeight / 2 -
            ...
            0.5 *
                (z.cylHeight / 2 -
                    (z.cylHeight / 2 - 80 + (z.cylHeight - 280) / 2))
    );
    fill(0, 200, 0, 80);
    cylinder(
        z.cylRadius - 1,
...
function drawTextForConstPressureSelection() {
    ...
    textSize(28);
    text(
        "pressure = " + z.pressureConstPressureCase.toFixed(2) + " bar",
        -265,
        -320
    );
    ...
    text(
        "mol B",
        90 + 450 / 3,
        305 - z.molBForConstPressureSelection * (530 / 7)
    );
    text("mol inerts", 62.5 + 450 * (2 / 3), 305 - z.molesInerts * (530 / 7));
    text(
        z.molAForConstPressureSelection.toFixed(1),
        110,
        265 - z.molAForConstPressureSelection * (530 / 7)
    );
    text(
        z.molBForConstPressureSelection.toFixed(1),
        110 + 450 / 3,
        265 - z.molBForConstPressureSelection * (530 / 7)
    );
    text(
        z.molesInerts.toFixed(1),
        110 + 450 * (2 / 3),
        265 - z.molesInerts * (530 / 7)
    );
...
function drawBarsForConstPressurePlot() {
    ...
    rect(
        60,
        310 - z.molAForConstPressureSelection * (530 / 7),
        450 / 3 - 10,
        z.molAForConstPressureSelection * (530 / 7)
    );
    ...
    rect(
        60 + 450 * (1 / 3),
        310 - z.molBForConstPressureSelection * (530 / 7),
        450 / 3 - 10,
        z.molBForConstPressureSelection * (530 / 7)
    );
...
function drawTextForConstVolumeSelection() {
    ...
    textSize(28);
    text(
        "final pressure = " + z.pressureConstVolumeCase.toFixed(2) + " bar",
        -265,
        -320
    );
    ...
    text(
        "mol B",
        90 + 450 / 3,
        305 - z.molBForConstVolumeSelection * (530 / 7)
    );
    text("mol inerts", 62.5 + 450 * (2 / 3), 305 - z.molesInerts * (530 / 7));
    text(
        z.molAForConstVolumeSelection.toFixed(1),
        110,
        265 - z.molAForConstVolumeSelection * (530 / 7)
    );
    text(
        z.molBForConstVolumeSelection.toFixed(1),
        110 + 450 / 3,
        265 - z.molBForConstVolumeSelection * (530 / 7)
    );
    text(
        z.molesInerts.toFixed(1),
        110 + 450 * (2 / 3),
...
function drawBarsForConstVolumePlot() {
    ...
    rect(
        60,
        310 - z.molAForConstVolumeSelection * (530 / 7),
        450 / 3 - 10,
        z.molAForConstVolumeSelection * (530 / 7)
    );
    ...
    rect(
        60 + 450 * (1 / 3),
        310 - z.molBForConstVolumeSelection * (530 / 7),
        450 / 3 - 10,
        z.molBForConstVolumeSelection * (530 / 7)
    );
