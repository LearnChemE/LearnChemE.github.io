...
function interpolation(arr, x, index) {
  let y;
  if(x <= arr[0][0]) {
    y = arr[0][index]
  } else if(x >= arr[arr.length - 1][0]) {
    y = arr[arr.length - 1][index];
  } else {
    for(let i = 0; i < arr.length - 1; i++) {
      if(x >= arr[i][0] && x < arr[i + 1][0]) {
        y = arr[i][index] + ((x - arr[i][0]) / (arr[i + 1][0] - arr[i][0])) * (arr[i + 1][index] - arr[i][index]);
      }
    }
  }
  return y
...
gvs.Tsat = function(x) {
  return interpolation(Tsat_list, x, 1)
}

gvs.S = function(x, index) {
  return interpolation(S_list, x, index)
}

gvs.H = function(x, index) {
  return interpolation(H_list, x, index)
}

gvs.Hsuper = function(x) {
  return interpolation(H_super_list, x, 1)
}

gvs.Ssuper = function(x) {
  return interpolation(S_super_list, x, 1)
}

gvs.SHsuper = function(x, n) {
  return interpolation(SHsuper_list[n], x, 1)
}

gvs.HTsuper = function(x, n) {
  return interpolation(HTsuper_list[n], x, 1)
}

gvs.vol = function(x) {
  return interpolation(vol_list, x, 1)
}

gvs.partsat = function(x) {
  let y1, y2;
  for(let i = 0; i < partsat_list.length - 1; i++) {
    if(x >= partsat_list[i][0] && x < partsat_list[i + 1][0]) {
      y1 = partsat_list[i][1] + ((x - partsat_list[i][0]) / (partsat_list[i + 1][0] - partsat_list[i][0])) * (partsat_list[i + 1][1] - partsat_list[i][1]);
    }
    if(x < partsat_list[i][0] && x >= partsat_list[i + 1][0]) {
      y2 = partsat_list[i][1] + ((x - partsat_list[i][0]) / (partsat_list[i + 1][0] - partsat_list[i][0])) * (partsat_list[i + 1][1] - partsat_list[i][1]);
    }
  }
  return [y1, y2]
}