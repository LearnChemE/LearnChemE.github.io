...
class Sim {
    ...
    calculateYs() {
        this.YS = this.XS.map(x => math.pow(x * this.M * this.L, 0.25));
    }
    ...
    drawGraphs() {
        for (let i = 0; i < this.MAXCOEFFICIENTS; i++) {
            const level = this.NS[i] ** 2 / (8 * 1.67 * this.M * this.L) ** 2 * 6.62 ** 2 * 6.02;
            this.densitygc.drawLine(this.xbounds, [level,level], hsvToRgb(0,0,0), 1, [2, 2]);
        }
        //
        //
